...
| sqlContextProvider.constantClassNames       | ExecutionContextProviderに登録する定数クラスを指定。`,`で区切ることで複数指定可。 ex) jp.co.future.uroborosql.context.test.TestConsts                                                                                                                                                                     |
| sqlContextProvider.enumConstantPackageNames | ExecutionContextProviderに登録するEnum定数パッケージ名を指定。`,`で区切ることで複数指定可。ex) jp.co.future.uroborosql.context.test                                                                                                                                                                       |
